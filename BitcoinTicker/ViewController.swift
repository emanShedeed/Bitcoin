        ...
        Alamofire.request(url, method: .get)
           .responseJSON { response in
               if response.result.isSuccess
               {
                    ...
                    self.parseJsonandUpdateUI(json: bitcoinJSON)
...
//    /***************************************************************/
//    
    func parseJsonandUpdateUI(json : JSON) {
        if let Price = json["ask"].double {
        
            bitcoinPriceLabel.text=String(Price)+" "+symbol
                   }
        else{
        bitcoinPriceLabel.text="bitcoin price not avalialbe"
        }
